  ...
  useEffect(() => {
    const fetchJobDetail = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8081/api/v1/public/job-listing/${jobId}`)
...
const handleApplicationSubmit = async (e: React.FormEvent) => {
  ...
  try {
    const response = await fetch("http://localhost:8081/api/v1/public/user-application/apply", {
      method: "POST",
      body: formData,
        headers: {
