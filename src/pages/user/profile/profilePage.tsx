  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:8081/api/v1/public/user-general/${userGeneralId}`, {
          headers: {
...
const viewCvBlob = () => {
  ...
  verified: user?.verified ?? true  // ruajmë statusin ekzistues
      ...
      await axios.put(`http://localhost:8081/api/v1/public/user-general/${userGeneralId}`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
